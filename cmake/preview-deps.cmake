...
macro(set_preview_dependencies target_name)
    if(NOT PREVIEW)
        find_package(Qt5 QUIET ${QT_MINIMUM_VERSION} OPTIONAL_COMPONENTS WebKitWidgets)
        if(Qt5WebKitWidgets_FOUND)
            target_link_libraries(${target_name} PRIVATE Qt5::WebKitWidgets)
            target_compile_definitions(${target_name} PRIVATE USEQTWEBKIT)
            message(STATUS "Qt webkitwidgets module found!")
        else()
            find_package(Qt5 QUIET ${QT_MINIMUM_VERSION} OPTIONAL_COMPONENTS WebEngineWidgets)
        ...
        if(PREVIEW STREQUAL "useqtwebkit")
            find_package(Qt5 ${QT_MINIMUM_VERSION} REQUIRED WebKitWidgets)
            target_link_libraries(${target_name} PRIVATE Qt5::WebKitWidgets)
            target_compile_definitions(${target_name} PRIVATE USEQTWEBKIT)
        elseif(PREVIEW STREQUAL "useqtwebengine")
            find_package(Qt5 ${QT_MINIMUM_VERSION} REQUIRED WebEngineWidgets)
            target_link_libraries(${target_name} PRIVATE Qt5::WebEngineWidgets)
            target_compile_definitions(${target_name} PRIVATE USEQTWEBENGINE)
        else()
            target_compile_definitions(${target_name} PRIVATE NOPREVIEW)
